...
int main(int argc, char *argv[]) {
    ...
    InferencePlugin plugin = InferenceEngine::PluginDispatcher().getPluginByDevice(FLAGS_d);
    ...
    const size_t infer_width = inputDims[0];
    const size_t infer_height = inputDims[1];
    const size_t num_channels = inputDims[2];
    const size_t channel_size = infer_width*infer_height;
    const size_t full_image_size = channel_size*num_channels;
    ...
    const size_t input_width   = static_cast<size_t>(cap.get(CAP_PROP_FRAME_WIDTH));
    const size_t input_height  = static_cast<size_t>(cap.get(CAP_PROP_FRAME_HEIGHT));
    ...
    if ((output_width == input_width) && (output_height == input_height)) {
        pipeline_type = 1;  // Input frame is resized to infer resolution
    } else {
        if ((output_width == input_width) && (output_height == input_height)) {
            pipeline_type = 2;  // output is input resolution
        }
    }
    ...
    int nBatch = 0;
    size_t frames_in_output = batchSize;
    ...
    for (;;) {
        ...
        if (frames_in_output == 0) {
            throw std::logic_error("frames_in_output can't be equal to 0");
        }
        if (bshow_less) {
            if (FLAGS_fr > 0)
        ...
        for (size_t mb = 0; mb < frames_in_output; mb++) {
            ...
            size_t maxProposalCount = outputDims[2];

            //---------------------------
            // parse SSD output
            //---------------------------
            for (size_t c = 0; c < maxProposalCount; c++) {
